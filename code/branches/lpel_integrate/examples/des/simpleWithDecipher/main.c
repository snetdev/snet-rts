...
bool silent;
#define PRINT( ARG) if( !silent) ARG
...
{ 
  ...
  for( i=0; i<SACARGgetShape( in_bits, 0); i++) {
    ...
    PRINT( fprintf(stderr, "Block %d:\n", i);)
    for( j=0; j<64; j++) {
      current_block[j]= bit_array[(i*64)+j];
      PRINT( fprintf(stderr, "%d ", current_block[j]);)
    }
...
void *Reader( void *buf)
{
  ...
  while( !terminate) {
    ...
    if( SNetRecGetDescriptor( resrec) == REC_terminate) {
      ...
      PRINT( fprintf(stderr, "Enciphered Bits:\n");)
      for( i=0; i<shape; i++) {
        PRINT( fprintf(stderr, "%d ", cbit_array[i]);)
      }
      PRINT( fprintf(stderr, "\n");)
      ToString( cbit_array);
    }
