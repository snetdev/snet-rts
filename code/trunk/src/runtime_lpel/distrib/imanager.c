...
/** <!--********************************************************************-->
 * $Id: imanager.c 2892 2010-10-28 10:20:51Z dlp $
 *
 * @file imanager.c
 *
 ...
 * static void IManagerCreateInput(snet_stream_t *stream, int node, int index) : Create a new input thread.
 ...
 * static snet_stream_t *IManagerMatchIndexToUpdate(snet_queue_t *queue, snet_id_t op, int node) : Get buffered update message that matches the index message.
 * static void IOManagerPutUpdateToBuffer(snet_queue_t *queue, snet_id_t op, int node, snet_stream_t *stream) : Buffer unused update message.
...
typedef struct {
  lpel_stream_t *stream;  /**< Output stream. */
 ...
 */
static lpel_thread_t *master_thread;
...
static void IManagerInputTask( lpel_task_t *self, void *ptr) 
{
  ...
  void *buf;
  int buf_size;
  lpel_stream_desc_t *outstream = LpelStreamOpen( self, data->stream, 'w');
  int position;
  ...
  while(!terminate) {
    ...
    case REC_collect:
    case REC_sort_end:
      LpelStreamWrite( outstream, record);
      break;
...
/** <!--********************************************************************-->
 *
 * @fn  static void IManagerCreateInput(snet_stream_t *stream, int node, int index)
 *
...
static void IManagerCreateInput( snet_stream_t *stream, int node, int index)
{
...
static void IManagerCreateNetwork( void *op) 
{
  ...
  stream = (snet_stream_t*) LpelStreamCreate();
...
static snet_stream_t *IManagerMatchIndexToUpdate(snet_queue_t *queue, snet_id_t op, int node)
{
...
static void IOManagerPutUpdateToBuffer(snet_queue_t *queue, snet_id_t op, int node, snet_stream_t *stream)
{
  snet_ru_entry_t *ru;
